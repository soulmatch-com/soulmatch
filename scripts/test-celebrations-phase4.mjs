...
const routes = ['src/app/(en)/page.tsx', 'src/app/(en)/60th-marriage/page.tsx', 'src/app/(en)/70th-marriage/page.tsx', 'src/app/(en)/80th-marriage/page.tsx', 'src/app/(en)/plan/page.tsx', 'src/app/(en)/matrimony/page.tsx']
...
test('public page loads services through the server helper', async () => { const source = await read('src/app/(en)/celebrations/thirukadaiyur/page.tsx'); assert.match(source, /loadCelebrationServices/); assert.doesNotMatch(source, /requestableServices|serviceOptions|celebrationServices/) })
...
test('legacy ceremony routes redirect directly to short canonical routes', async () => assert.match(await read('src/app/(en)/celebrations/[slug]/page.tsx'), /permanentRedirect\(`\/\$\{slug\}`\)/))
test('legacy enquiry route redirects to the canonical plan route', async () => assert.match(await read('src/app/(en)/celebrations/enquire/page.tsx'), /\/plan/))
