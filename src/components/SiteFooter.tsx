...
function SocialLinks() {
  ...
  return <nav aria-label="Follow MyThirumanam" className="mt-5"><p className="text-sm font-semibold text-slate-900 dark:text-white">Follow us</p><ul className="mt-3 flex flex-wrap gap-2">{availableLinks.map(([label, href, Icon]) => <li key={label}><a href={href} target="_blank" rel="noopener noreferrer" aria-label={`Follow MyThirumanam on ${label}`} className="inline-flex size-9 items-center justify-center rounded-full border border-slate-300 text-slate-600 transition-colors hover:border-amber-700 hover:bg-amber-50 hover:text-amber-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-700 dark:border-slate-700 dark:text-slate-300 dark:hover:border-amber-300 dark:hover:bg-slate-900 dark:hover:text-amber-300"><Icon aria-hidden="true" className="size-4" /></a></li>)}</ul></nav>
}
