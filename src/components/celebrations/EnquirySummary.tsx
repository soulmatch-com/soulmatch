...
export function EnquiryConfirmation({ enquiryId, values, ceremony }: { enquiryId: string; values: CelebrationEnquiryFormValues; ceremony: Ceremony }) {
  const [copyMessage, setCopyMessage] = useState('')
  const copyReference = async () => {
    try {
      await navigator.clipboard.writeText(enquiryId)
  ...
  return <section aria-labelledby="enquiry-success-title" className="rounded-3xl border border-emerald-200 bg-white p-6 shadow-lg sm:p-10">
    ...
    <p className="mt-6 text-sm text-stone-600">Reference:</p><p className="mt-1 select-all break-all font-mono font-semibold">{enquiryId}</p>
