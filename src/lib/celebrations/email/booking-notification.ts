...
export function createBookingNotification({ enquiryId, enquiry, services }: { enquiryId: string; enquiry: CelebrationEnquiryApiInput; services: PublicCelebrationService[] }) {
  ...
  const sections = [
    ...
    ['Other Requirements', optional(enquiry.otherServiceDetails)], ['Additional Notes', optional(enquiry.notes)],
  ] as const
...
export async function sendBookingNotification(input: { enquiryId: string; enquiry: CelebrationEnquiryApiInput; services: PublicCelebrationService[] }) {
