...
export async function checkSharedRateLimit({ url, token, key, capacity, windowMs, request = fetch }: RateLimitStoreConfig): Promise<RateLimitResult> {
  const response = await request(url, {
    method: 'POST',
    headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
    body: JSON.stringify(['EVAL', TOKEN_BUCKET_SCRIPT, '1', key, String(capacity), String(windowMs)]),
    cache: 'no-store',
  })
