-- Corrects the worker claim RPC output-column ambiguity without changing its signature.
-- Apply after fix_notification_queue_campaign_ambiguity.sql.
...
BEGIN
  ...
  WITH candidates AS (
    ...
    WHERE status = 'queued' AND id IN (SELECT claimed.campaign_id FROM claimed)
  )
